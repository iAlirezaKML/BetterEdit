...
class MultiLineFuncCommand: NSObject, XCSourceEditorCommand {
	typealias ParsedPayload = (
		indent: String,
		funcName: String,
		args: [String],
		trailingText: String
	...
	func perform(
		...
		guard firstLine <= lastLine,
			firstLine >= 0, lastLine < invocation.buffer.lines.count
		else { return }

		putEachArgumentOnItsOwnLine(
			invocation.buffer.lines,
			in: firstLine ... lastLine
		)
	}

	private func putEachArgumentOnItsOwnLine(
		_ allLines: NSMutableArray,
	...
	private func parse(_ originalText: String) -> ParsedPayload {
		// Get indentation
		var indent = ""
